...
#property description "v3.12: Per-account API key for multi-user support."
#property strict
...
input string   InpAccountKey  = "";                                       // Account API Key (copy from Settings page)
input int      InpAccountId   = 0;                                        // [DEPRECATED] Legacy Account ID (use API Key)
...
{
   Print("============================================");
   Print("  AutoJournaler v3.12 (Universal) Started");
   Print("  Webhook: ", InpApiUrl);
   Print("  Account Key: ", StringLen(InpAccountKey) > 0 ? StringSubstr(InpAccountKey, 0, 8) + "..." : "(not set)");
   Print("  Legacy Account ID: ", InpAccountId > 0 ? IntegerToString(InpAccountId) : "(not used)");
   ...
   if(InpAccountKey == "" && InpAccountId <= 0)
   {
      Print("WARNING: Neither InpAccountKey nor InpAccountId is set. Trades will NOT be recorded.");
      Print("         Copy your Account API Key from the Settings page and paste it here.");
   }
   if(InpAccountKey == "" && InpAccountId > 0)
   {
      Print("NOTE: Using deprecated InpAccountId. For reliability, switch to InpAccountKey (copy from Settings page).");
   }
...
void SyncMissedDeals()
{
   ...
   // Prefer accountKey (UUID) over legacy numeric ID
   string json = "{\"tickets\":" + ticketsJson;
   if(StringLen(InpAccountKey) > 0)
      json += ",\"accountKey\":\"" + InpAccountKey + "\"";
   else if(InpAccountId > 0)
