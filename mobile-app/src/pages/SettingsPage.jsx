...
export default function SettingsPage() {
  ...
  const handleCopyApiKey = async (account) => {
    ...
    try {
      ...
      textarea.value = account.api_key;
  ...
  const handleRegenerateKey = async (account) => {
    const confirmed = window.confirm(
      `Regenerate the API key for "${account.name}"?\n\nThe old key will stop working immediately. You'll need to update the key in your MT5 EA.`
    );
    if (!confirmed) return;
    ...
    if (!result.success) {
      window.alert(result.error || 'Failed to regenerate API key.');
    }
  };
              ...
              >
                ...
                <div className="api-key-section">
                  <button
                    ...
                    onClick={() =>
                      setExpandedApiKeyId(
                        expandedApiKeyId === account.id ? null : account.id
                      )
                    }
                  >
                    <IoKeyOutline size={16} />
                    <span>MT5 API Key</span>
                    <span className="api-key-toggle__chevron">
                  ...
                  {expandedApiKeyId === account.id && (
                    <div className="api-key-details">
                      <div className="api-key-value">
                        <code>{account.api_key || 'Generating...'}</code>
                        <button
                          type="button"
                          className="icon-button"
                          title="Copy API key"
                          onClick={() => handleCopyApiKey(account)}
                        >
                          {copiedKeyId === account.id ? (
                            <IoCheckmarkCircle size={18} color="#50C878" />
                          ) : (
                      ...
                      {copiedKeyId === account.id && (
                        <span className="api-key-copied">Copied!</span>
                      )}
                      <p className="api-key-help">
                        Paste this key into your MT5 EA's <strong>Account API Key</strong> input.
                        This key uniquely identifies this account.
                      </p>
                      <button
                        type="button"
                        className="ghost-button ghost-button--small"
                        onClick={() => handleRegenerateKey(account)}
                        disabled={regeneratingKeyId === account.id}
                      >
                        <IoRefresh size={14} />
                        {regeneratingKeyId === account.id
                          ? 'Regenerating...'
                          : 'Regenerate Key'}
                      </button>
                    </div>
