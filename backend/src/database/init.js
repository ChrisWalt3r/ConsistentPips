...
const createTables = async () => {
  const tables = [
    ...
    `CREATE TABLE IF NOT EXISTS journal_entries (
      ...
      before_image_filename TEXT,
      after_image_filename TEXT,
...
const createIndexes = async () => {
  ...
  const indexes = [
    ...
    'CREATE UNIQUE INDEX IF NOT EXISTS idx_accounts_api_key ON accounts(api_key)'
  ];
...
const runLiveMigrations = async () => {
  const migrations = [
    ...
    // Per-account API key: stable UUID used by MT5 EA to route trades to the correct account
    `ALTER TABLE accounts ADD COLUMN IF NOT EXISTS api_key UUID DEFAULT gen_random_uuid()`,
    // Backfill existing accounts that don't have an API key yet
    `UPDATE accounts SET api_key = gen_random_uuid() WHERE api_key IS NULL`,
