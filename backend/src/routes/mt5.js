...
router.post('/check-tickets', verifyWebhookSecret, async (req, res) => {
    try {
        const { tickets, accountId: rawAccountId, accountKey } = req.body;

        if (!tickets || !Array.isArray(tickets) || tickets.length === 0) {
            return res.status(400).json({ error: 'tickets array is required' });
        }

        // Resolve the real account ID: prefer stable accountKey (UUID), fall back to numeric ID
        let resolvedAccountId = null;
        if (accountKey) {
            const row = await getRow('SELECT id FROM accounts WHERE api_key = $1', [accountKey]);
            if (!row) {
                return res.status(404).json({ error: `No account found for the provided API key. Check your EA settings.` });
            }
            resolvedAccountId = row.id;
        } else if (rawAccountId) {
            resolvedAccountId = parseInt(rawAccountId);
            const row = await getRow('SELECT id FROM accounts WHERE id = $1', [resolvedAccountId]);
            if (!row) {
                return res.status(404).json({ error: `Account ID ${resolvedAccountId} not found. Use Account API Key instead.` });
            }
        } else {
            return res.status(400).json({ error: 'accountKey or accountId is required in request body' });
        }
...
router.post('/webhook', verifyWebhookSecret, upload.single('image'), async (req, res) => {
    try {
        const {
            ...
            accountId: rawAccountId, // Legacy numeric ID (deprecated)
            accountKey // Stable UUID API key (preferred)
        } = req.body;
        ...
        // Resolve account: prefer stable accountKey (UUID), fall back to numeric accountId
        // No more "pick the latest account" fallback — that causes cross-user trade pollution
        let accountId = null;
        if (accountKey) {
            const row = await getRow('SELECT id FROM accounts WHERE api_key = $1', [accountKey]);
            if (!row) {
            ...
            accountId = parseInt(rawAccountId);
            const row = await getRow('SELECT id FROM accounts WHERE id = $1', [accountId]);
            if (!row) {
                return res.status(404).json({ error: `Account ID ${accountId} not found. Use Account API Key instead — copy it from your Settings page.` });
            }
        } else {
            return res.status(400).json({ error: 'accountKey or accountId is required. Copy your Account API Key from Settings.' });
        }
