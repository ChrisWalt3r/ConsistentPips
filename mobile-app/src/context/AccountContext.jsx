  ...
  const value = useMemo(
    () => ({
      ...
      async regenerateApiKey(accountId) {
        try {
          const response = await accountsAPI.regenerateApiKey(accountId);
          await fetchAccounts();
          return { success: true, apiKey: response.data?.api_key };
        } catch (error) {
          console.error('AccountContext: failed to regenerate API key', error);
          return {
            success: false,
            error: error?.data?.error || 'Failed to regenerate API key',
          };
        }
      },
    }),
