            ...
            accounts.map(account => (
              ...
              >
                ...
                <TouchableOpacity
                  ...
                  onPress={() => setExpandedApiKeyId(expandedApiKeyId === account.id ? null : account.id)}
                >
                  <Ionicons name="key-outline" size={16} color="#666" />
                  <Text style={styles.apiKeyToggleText}>MT5 API Key</Text>
                  <Ionicons name={expandedApiKeyId === account.id ? "chevron-up" : "chevron-down"} size={14} color="#999" />
                </TouchableOpacity>

                {expandedApiKeyId === account.id && (
                  <View style={styles.apiKeyDetails}>
                    <View style={styles.apiKeyValueRow}>
                      <Text style={styles.apiKeyValue} selectable>
                        {account.api_key || 'Generating...'}
                      </Text>
                      <TouchableOpacity
                        style={styles.apiKeyCopyButton}
                        onPress={async () => {
                          if (account.api_key) {
                            await Clipboard.setStringAsync(account.api_key);
                      ...
                      >
                        <Ionicons
                          name={copiedKeyId === account.id ? "checkmark-circle" : "copy-outline"}
                          size={18}
                          color={copiedKeyId === account.id ? "#50C878" : "#4A90E2"}
                        />
                      </TouchableOpacity>
                    </View>
                    {copiedKeyId === account.id && (
                      <Text style={styles.apiKeyCopied}>Copied!</Text>
                    )}
                    <Text style={styles.apiKeyHelp}>
                      Paste this key into your MT5 EA's "Account API Key" input. This key uniquely identifies this account.
                    </Text>
                    <TouchableOpacity
                      style={styles.regenerateButton}
                      disabled={regeneratingKeyId === account.id}
                      onPress={() => {
                        Alert.alert(
                          'Regenerate Key',
                          `Regenerate the API key for "${account.name}"?\n\nThe old key will stop working immediately.`,
                          [
                            { text: 'Cancel', style: 'cancel' },
                            {
                              text: 'Regenerate',
                              style: 'destructive',
                              onPress: async () => {
                                setRegeneratingKeyId(account.id);
                                const result = await regenerateApiKey(account.id);
                                setRegeneratingKeyId(null);
                                if (!result.success) {
                                  Alert.alert('Error', result.error || 'Failed to regenerate API key.');
                                }
                              }
                            }
                          ]
                    ...
                    >
                      <Ionicons name="refresh" size={14} color="#4A90E2" />
                      <Text style={styles.regenerateButtonText}>
                        {regeneratingKeyId === account.id ? 'Regenerating...' : 'Regenerate Key'}
                      </Text>
                    </TouchableOpacity>
                  </View>
                )}
