...
router.get('/', async (req, res) => {
  try {
    ...
    const accounts = await getAllRows(
      `SELECT id, name, description, color, starting_balance, is_active, api_key, created_at, updated_at 
         FROM accounts 
...
// POST /api/accounts/:id/regenerate-key - Regenerate API key for an account (user-scoped)
router.post('/:id/regenerate-key', async (req, res) => {
  try {
    ...
    const result = await runQuery(
      `UPDATE accounts SET api_key = gen_random_uuid(), updated_at = NOW()
